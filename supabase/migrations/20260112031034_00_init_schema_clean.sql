/*
  # UhaiLink Schema - Fresh Initialize
*/
...
CREATE POLICY "owner_full_access" ON public.profiles FOR ALL TO authenticated
  USING (auth.uid() = id) WITH CHECK (auth.uid() = id);

CREATE POLICY "admin_full_access" ON public.profiles FOR ALL TO authenticated
...
CREATE INDEX IF NOT EXISTS idx_qr_token_active ON public.qr_access_tokens(access_token, is_active) WHERE is_active = true;

CREATE POLICY "owner_qr_access" ON public.qr_access_tokens FOR ALL TO authenticated
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE POLICY "public_qr_access" ON public.profiles FOR SELECT TO public
  USING (EXISTS (SELECT 1 FROM public.qr_access_tokens WHERE user_id = profiles.id AND is_active = true));
...
CREATE POLICY "org_public_read" ON public.emergency_organizations FOR SELECT TO authenticated USING (true);
CREATE POLICY "admin_manage_orgs" ON public.emergency_organizations FOR ALL TO authenticated
  USING (public.is_admin(auth.uid())) WITH CHECK (public.is_admin(auth.uid()));
...
CREATE POLICY "tut_public_read" ON public.tutorials FOR SELECT TO authenticated USING (true);
CREATE POLICY "admin_manage_tutorials" ON public.tutorials FOR ALL TO authenticated
  USING (public.is_admin(auth.uid())) WITH CHECK (public.is_admin(auth.uid()));
...
BEGIN
  ...
  VALUES (NEW.id, NEW.email, COALESCE(NEW.raw_user_meta_data->>'full_name', ''), NEW.raw_user_meta_data->>'phone', 'user');
  INSERT INTO public.qr_access_tokens (user_id, access_token) VALUES (NEW.id, gen_random_uuid()::text);
  RETURN NEW;
END;
$$;
...
BEGIN
  ...
  IF u_id IS NULL THEN RETURN json_build_object('success', false, 'message', 'User not found'); END IF;
